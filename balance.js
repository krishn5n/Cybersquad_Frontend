...
addearning.addEventListener('click', async () => {
    let url = 'http://127.0.0.1:5000/addinflux'
    try {
...
async function influxload() {
    let url = 'http://127.0.0.1:5000/influxlist'
    try {
        let response = await fetch(url)
...
async function latestspends() {
    let url = 'http://127.0.0.1:5000/latestspend'
...
async function bargraph() {
    try {
        ...
        let url = 'http://127.0.0.1:5000/bargraph'
        let dict = { 'email': email }
        let response = await fetch(url, {
            method: 'POST',
            headers: {
